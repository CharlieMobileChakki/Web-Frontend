...
const ProductTable = ({ products = [], categories = [], onEdit, onDelete, currentPage, totalPages, onPageChange, totalRecords = 0, itemsPerPage = 10 }) => {
    const nameRefs = useRef({});
    const [isNameOverflow, setIsNameOverflow] = useState({});
    const [expandedVariants, setExpandedVariants] = useState({});

    const getCategoryName = (category) => {
        // Handle if category is already an object (as per new API response)
    ...
    // Pagination constants (used for SR NO)
    const startIndex = (currentPage - 1) * itemsPerPage;
    const remainingCount = Math.max(0, totalRecords - (currentPage * itemsPerPage));
    ...
    // products change hone par check overflow
    useEffect(() => {
        const checkOverflow = () => {
    ...
    return (
        <div className="space-y-4">
            {/* Status Indicator */}
            <div className="flex flex-wrap items-center justify-between px-2 text-sm">
                <div className="flex items-center gap-4 text-gray-600">
                    <span className="flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-blue-500"></span>
                        Total: <span className="font-bold text-gray-900">{totalRecords}</span>
                    </span>
                    <span className="flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-orange-400"></span>
                        Remaining: <span className="font-bold text-gray-900">{remainingCount}</span>
                    </span>
                </div>
